...
// PlannerPrompt gera um prompt para o modelo criar um plano de execução de ferramentas
...
- a mensagem contém ações não suportadas
- a mensagem está confusa ou sem ação clara
- a confiança na interpretação é menor que 0.6

Se a mensagem contiver comandos não suportados,
IGNORE essas partes e execute apenas as ações
que correspondem a ferramentas disponíveis.

Use "unknown" apenas quando nenhuma ação da
mensagem puder ser executada.
